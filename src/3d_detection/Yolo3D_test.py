...
## path
path = edict()
path.data_path = r"D:\Projects\SEAR\datasets\training\training" # used in visualDet3D/data/.../dataset
path.test_path = r"D:\Projects\SEAR\datasets\training\training" # used in visualDet3D/data/.../dataset
path.visualDet3D_path = r"D:\CODE\visualDet3D\visualDet3D" # The path should point to the inner subfolder
path.project_path = r"D:\CODE\visualDet3D/temp" # or other path for pickle files, checkpoints, tensorboard logging and output files.
if not os.path.isdir(path.project_path):
    os.mkdir(path.project_path)
path.project_path = os.path.join(path.project_path, 'train_test_sndd')
if not os.path.isdir(path.project_path):
    os.mkdir(path.project_path)
...
data = edict(
    ...
    train_split_file = os.path.join(cfg.path.visualDet3D_path, '../', 'temp', 'training_split.txt'),
    val_split_file   = os.path.join(cfg.path.visualDet3D_path, '../', 'temp',  'validation_split.txt'),
)
