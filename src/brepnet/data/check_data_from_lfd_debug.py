...
def process_pkl(pkl_file, debug_folder_list=None):
    ...
    for i, j in list(zip(idx0, idx1)):
        if debug_folder_list and (local_folder_list[i] not in debug_folder_list and local_folder_list[j] not in debug_folder_list):
            continue
        if i < j:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lfd', type=float, required=False, default=100)
    ...
    if args.prefix.endswith("txt"):
        if not os.path.exists(args.prefix):
            raise ValueError(f"File {args.prefix} does not exist.")
        with open(args.prefix, "r", encoding="utf-8") as f:
            lines = f.readlines()
            debug_folder_list = [line.strip() for line in lines]
            print(f"Debug folder list: {len(debug_folder_list)}")
            pass
    else:
        debug_folder_list = args.prefix

    out_txt_path = pkl_root + f"brepgen_train_novel_lfd_{lfd_threshold}_{debug_folder_list[0]}.txt"
    ...
    for pkl_file in tqdm(all_pkl_files):
        if not pkl_file.endswith(".pkl"):
            continue
        futures.append(process_pkl_remote.remote(os.path.join(pkl_root, pkl_file), debug_folder_list))
    ...
    deduplicated_txt_path = pkl_root + f"brepgen_train_deduplicated_lfd_{lfd_threshold}_{debug_folder_list[0]}.txt"
    ...
    remove_folder_txt_path = pkl_root + f"brepgen_train_removed_lfd_{lfd_threshold}_{debug_folder_list[0]}.txt"
    with open(remove_folder_txt_path, "w") as f:
