...
def get_brep(v_root, output_root, v_folder):
    ...
    try:
        ...
        face_dict_filtered = {}
        face_dict_filtered_normal = {}
        for face in face_dict_geom_type:
            if face_dict_geom_type[face] == GeomAbs_Plane:
                face_dict_filtered[face] = face_dict[face]
                # normal1 and normal2 should be the same
        ...
        for i in range(len(face_dict_filtered_list)):
            for j in range(i+1, len(face_dict_filtered_list)):
                ...
                face2 = face_dict_filtered_list[j]
                # Check normal
