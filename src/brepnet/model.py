...
from src.brepnet.dataset import denormalize_coord
...
class AutoEncoder_1119_light(nn.Module):
    def __init__(self, v_conf):
        ...
        self.bbox_encoder = nn.Sequential(
                nn.Linear(6, bd),
                nn.LeakyReLU(),
                nn.TransformerEncoder(attn_layer, 4),
                nn.Linear(bd, df),
        )
        self.bbox_encoder_to_edge = nn.Linear(df, 2*df)

        self.edge_feat_attn = nn.Sequential(
                nn.Linear(df*2, bd),
                nn.TransformerEncoder(attn_layer, 4),
                nn.Linear(bd, df*2)
        )
        ...
        self.inter = AttnIntersection3(df, 8 + df*2, 512, 8)
        self.classifier = nn.Linear(8, 1)
        self.inter_p = AttnIntersection3(df, 8, 512, 4)
        self.classifier_p = nn.Linear(8, 1)
        self.inter_v = AttnIntersection3(df, 8, 512, 4)
        self.classifier_v = nn.Linear(8, 1)

        self.face_attn_proj_in2 = nn.Linear(df, bd)
        self.face_attn_proj_out2 = nn.Linear(bd, df)
        self.face_attn2 = nn.TransformerEncoder(attn_layer, 4)
        ...
        self.face_center_scale_decoder = nn.Sequential(
                nn.Linear(dl * 2 * 2, bd),
                nn.TransformerEncoder(attn_layer, 8),
                nn.Linear(bd, 6),
        )
    ...
    def encode(self, v_data, v_test):
        ...
        face_bbox_features = self.bbox_encoder(v_data["face_bbox"])
        edge_features = self.edge_coords(edge_points)
        edge_bbox_features = self.bbox_encoder_to_edge(self.bbox_encoder(v_data["edge_bbox"]))
        edge_features = self.edge_feat_attn(edge_features)

        face_features = face_features + face_bbox_features
        edge_features = edge_features + edge_bbox_features
    ...
    def decode(self, v_encoding_result, v_data=None, v_deduplicated=False):
        src_face_z = v_encoding_result["face_z"]
        face_feature = self.face_attn_proj_in2(src_face_z)
        if v_data is None:
            num_faces = src_face_z.shape[0]
            attn_mask = torch.zeros((num_faces, num_faces), dtype=bool, device=src_face_z.device)
        else:
        ...
        if v_deduplicated: # Deduplicate
            ...
            pred_face_points = denormalize_coord(face_points_local, face_center_scale)
        ...
        if v_data is None:
            ...
            cls_feature, feature_pair = feature_pair[:, :8], feature_pair[:, 8:]
            pred = self.classifier(cls_feature)[...,0]
            ...
            feature_pair = self.inter(feature_pair)
            cls_feature, feature_pair = feature_pair[:, :8], feature_pair[:, 8:]
            pred = self.classifier(cls_feature)
            ...
            loss_parallel = 0 if torch.isnan(loss_parallel) else loss_parallel
            ...
            loss_vertical = 0 if torch.isnan(loss_vertical) else loss_vertical
    ...
    def forward(self, v_data, v_test=False):
        ...
        if v_test:
            ...
            pred_edge = denormalize_coord(pred_data["edge_points_local"], pred_data["edge_center_scale"])
            ...
            loss["edge_coords"] = nn.functional.l1_loss(
                denormalize_coord(decoding_result["edge_points_local"], decoding_result["edge_center_scale"])[..., :3],
                v_data["edge_points"][v_data["edge_face_connectivity"][:, 0]][..., :3]
            )

            loss["edge_coords1"] = nn.functional.l1_loss(
                denormalize_coord(decoding_result["edge_points_local1"], decoding_result["edge_center_scale1"])[..., :3],
                v_data["edge_points"][..., :3]
            )
            data["gt_face"] = v_data["face_points"].detach().cpu().numpy()
            pred_face = denormalize_coord(pred_data["face_points_local"], pred_data["face_center_scale"])
