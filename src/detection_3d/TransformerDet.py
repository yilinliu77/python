...
class Swin_Transformer(nn.Module):
    def __init__(self, v_checkpoint_path=False,v_is_tiny=True):
...
class Swin_Transformer_FPN(nn.Module):
    def __init__(self, v_checkpoint_path=False,v_is_tiny=True,v_output_features=256):
        super(Swin_Transformer_FPN, self).__init__()
        if v_is_tiny:
            self.swin_transformer = SwinTransformer(pretrain_img_size=224)
            self.fpn = torchvision.ops.feature_pyramid_network.FeaturePyramidNetwork([96, 192, 384, 768], v_output_features)
            ...
            self.swin_transformer = SwinTransformer(pretrain_img_size=224,
                                                    ...
                                                    num_heads=[4, 8, 16, 32],
                                                    drop_path_rate=0.3, )
            self.fpn = torchvision.ops.feature_pyramid_network.FeaturePyramidNetwork([128, 256, 512, 1024], v_output_features)
