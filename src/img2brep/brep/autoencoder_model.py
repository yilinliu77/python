...
class AutoEncoder(nn.Module):
    ...
    def inference(self, v_face_embeddings, return_topology=False):
        ...
        if gathered_edge_features.shape[0] < 128 * 128:
    ...
    def forward(self, v_data,
        ...
        if return_true_loss:
            ...
            true_recon_face_loss = nn.functional.l1_loss(
                data["recon_faces"][face_mask], v_data["face_points"][face_mask], reduction='mean')
        ...
        if return_face_features:
            face_embeddings_return = v_data["atten_face_embeddings"].new_zeros(
                (*face_mask.shape, v_data["atten_face_embeddings"].shape[-1]))
            face_embeddings_return = face_embeddings_return.masked_scatter(
                rearrange(face_mask, '... -> ... 1'), v_data["atten_face_embeddings"])
...
class AutoEncoder_derect_discrete(nn.Module):
    def __init__(self,
                 v_conf,
                 ):
        super(AutoEncoder_derect_discrete, self).__init__()
        self.dim_shape = v_conf["dim_shape"]
        self.dim_latent = v_conf["dim_latent"]
        self.with_quantization = v_conf["with_quantization"]
        self.finetune_decoder = v_conf["finetune_decoder"]
    ...
    def forward(self, v_data,
                ...
                return_true_loss=False,
                **kwargs):
        face_mask = (v_data["discrete_face_points"]!=-1).all(dim=-1).all(dim=-1).all(dim=-1)
        face_features = self.face_coords(v_data["discrete_face_points"][face_mask])

        edge_mask = (v_data["discrete_edge_points"]!=-1).all(dim=-1).all(dim=-1)
        edge_features = self.edge_coords(v_data["discrete_edge_points"][edge_mask])
        ...
        if return_recon:
            ...
            edge_coords = (pre_edge_coords.argmax(dim=-1) - coord_shifts) / coord_shifts
            vertex_coords = (pre_vertex_coords.argmax(dim=-1) - coord_shifts) / coord_shifts
