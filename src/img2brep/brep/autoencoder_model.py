...
class AutoEncoder4(nn.Module):
    ...
    def forward(self, v_data,
                ...
                return_true_loss=False,
                **kwargs):
        face_coords, edge_coords, vertex_coords, face_mask, edge_mask, vertex_mask = self.encoder(v_data)
        edge_feature = self.edge_encoder(flatten_edge_points)

        loss = {}

        # Decode with normal feature
        predicted_face_points = self.decoder.decode(true_face_latent).sample
        # =============================== Loss for normal decoding ===============================
        loss["face_points"] = nn.functional.l1_loss(flatten_face_points, predicted_face_points)
        if self.with_quantization:
            loss["total_loss"] = loss["face_points"]
