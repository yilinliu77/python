...
class Autoencoder_Dataset(torch.utils.data.Dataset):
    ...
    def collate_fn(batch):
        (sample_points_faces, sample_points_lines, sample_points_vertices, face_edge_loop, face_adj,
         edge_face_connectivity, vertex_edge_connectivity) = zip(*batch)
...
class Autoregressive_Dataset(torch.utils.data.Dataset):
    def __init__(self, v_training_mode, v_conf):
        super(Autoregressive_Dataset, self).__init__()
        self.mode = v_training_mode
        self.conf = v_conf
        self.dataset_path = v_conf['root']
        self.is_overfit = v_conf['overfit']
    ...
    def collate_fn(batch):
        ...
        face_idx_sequence = pad_sequence(face_idx_sequence, batch_first=True, padding_value=-1)
