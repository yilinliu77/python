...
class Auotoencoder_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        face_edges_idx = torch.from_numpy(data_npz['face_edge_idx']).to(torch.int).to(self.device)
        ...
        edge_adj = torch.from_numpy(data_npz['edge_adj']).to(torch.int).to(self.device)
