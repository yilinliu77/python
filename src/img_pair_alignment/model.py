...
class NeuralImageFunction(nn.Module):
    ...
    def visualize(self, v_prediction, v_gt, v_raw_img, v_step: int):
        ...
        with torch.no_grad():
            ...
            corners[..., 0] = (corners[..., 0] / self.raw_img_width * max(self.raw_img_height,
                                                                                self.raw_img_width) + 1) / 2 * self.raw_img_width - 0.5
            corners[..., 1] = (corners[..., 1] / self.raw_img_height * max(self.raw_img_height,
                                                                                 self.raw_img_width) + 1) / 2 * self.raw_img_height - 0.5
