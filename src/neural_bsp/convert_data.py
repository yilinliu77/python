...
def reader(v_queue: Queue, v_features, v_flags, v_points, num_block):
    cur_id = 0
    while cur_id < len(v_features):
        ...
        v_queue.put((cur_id, prefix, feat, flag, points))
        cur_id += 1
...
def writer(v_queue: Queue, v_num_files, v_filename, num_block):
    ...
    while True:
        ...
        idx, prefix, feat, flag, points = (
            [None] * num_block, [None] * num_block, [None] * num_block, [None] * num_block, [None] * num_block)
        for i in range(num_block):
            idx[i], prefix[i], feat[i], flag[i], points[i] = v_queue.get()
...
if __name__ == '__main__':
    ...
    p_reader = Process(target=reader, args=(q, feature_files, flag_files, point_files, num_block))
