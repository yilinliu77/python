...
from src.neural_bsp.attention_unet_3d import AttU_Net_3D, U_Net_3D
...
class ABC_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cur_time = time.time()
        consistent_flags = torch.from_numpy(np.unpackbits(data["consistent_flags"]).reshape(
            resolution, resolution, resolution, 26))
...
class Base_model(nn.Module):
    def __init__(self, v_phase=0):
        ...
        self.phase = v_phase
        self.encoder = U_Net_3D(img_ch=4, output_ch=26)
        # self.encoder = AttU_Net_3D(img_ch=4, output_ch=26)
...
class Base_phase(pl.LightningModule):
    def __init__(self, hparams, v_data):
        ...
        source_coords, target_coords, valid_flag = construct_graph_3d(128)
        source_coords = ((source_coords / 127) * 2 - 1).astype(np.float32)
        self.viz_data = {
            "query_points": source_coords,
            "target_vertices": target_coords,
