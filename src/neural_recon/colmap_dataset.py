...
class Colmap_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Read original img
        if False:
            original_imgs = [cv2.resize(cv2.cvtColor(cv2.imread(item.img_path, cv2.IMREAD_UNCHANGED)[:,:,:3], cv2.COLOR_BGR2RGB), (800,600)) for item in visible_imgs]
        ...
        projection_matrix = torch.from_numpy(np.stack(projection_matrix_,axis=0).astype(np.float32)) if len(projection_matrix_) > 1 else 0
