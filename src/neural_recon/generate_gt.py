...
def compute_direction_similarity(v_segment: np.ndarray, v_direction_img: np.ndarray):
    ...
    angle_cos1 = np.dot(sample_direction, source_direction) / (
            1e-8 + np.linalg.norm(sample_direction, axis=1) * np.linalg.norm(source_direction))
    angle_cos2 = np.dot(sample_direction, -source_direction) / (
            1e-8 + np.linalg.norm(sample_direction, axis=1) * np.linalg.norm(-source_direction))
    angle_cos = np.max(np.asarray((angle_cos1, angle_cos2)), axis=0)
    return np.mean(angle_cos), np.mean(np.linalg.norm(sample_direction,axis=1))
...
def extract_roi_region(v_img: np.ndarray, v_roi_center, v_roi_angle, v_roi_size):
    ...
    if False:
        ...
        img_viz = cv2.line(img, projected_segment_per_view_original[0], projected_segment_per_view_original[1],
                           (0, 0, 255), 5)
