...
from src.neural_recon.models import Explorer
...
class Phase2(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.bounds_center = (bounds_min[0] + bounds_max[1]) / 2
        self.bounds_size = (bounds_max[1] - bounds_min[0]).max()
        ...
        self.img_models = self.prepare_model(self.data["img_database"], self.hydra_conf["dataset"]["img_nif_dir"])
    ...
    def prepare_dataset(self, v_colmap_dir, v_bounds):
        ...
        if True:
            ...
            sdf_computer.setup_mesh(data["gt_mesh_vertices"][data["gt_mesh_faces"]], False)  # Do not automatically compute the bounds
            ...
            pool = Pool(16)
            check_visibility(np.zeros((4,4),dtype=np.float32), np.zeros((2,3),np.float32)) # Dummy, just for compiling the function
    ...
    def prepare_model(self, v_imgs, v_nif_dir):
        ...
        def load_img_model(img_name):
            if os.path.exists(os.path.join(v_nif_dir, img_name)):
                checkpoint_name = [item for item in os.listdir(os.path.join(v_nif_dir, img_name)) if item[-4:]=="ckpt"]
    ...
    def train_dataloader(self):
        self.train_dataset = Colmap_dataset(
            self.data,
            "validation",
        )
    ...
    def training_step(self, batch, batch_idx):
        predicted_prediction, gt_prediction = self.model(batch)
        loss = self.model.loss(predicted_prediction, gt_prediction)

        self.log("Training_Loss", loss.detach(), prog_bar=True, logger=True, on_step=True, on_epoch=True,
                 batch_size=batch["sample_point"].shape[0])
...
@hydra.main(config_name="phase2_test.yaml", config_path="../../configs/neural_recon/", version_base="1.1")
