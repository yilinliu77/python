...
class LModel17(nn.Module):
    ...
    def sample_points_based_on_up(self, edge_points, edge_up_c, v_id_epoch, is_log):
        ...
        coord_2d1 = (self.intrinsic1 @ interpolated_coordinates_camera.T).T
        coord_2d1 = coord_2d1[:, :2] / coord_2d1[:, 2:3]
        valid_mask1 = torch.logical_and(coord_2d1 > 0, coord_2d1 < 1)
        valid_mask1 = torch.logical_and(valid_mask1[:, 0], valid_mask1[:, 1])
        coord_2d1 = torch.clamp(coord_2d1, 0, 1)
        time_profile[4], timer = refresh_timer(timer)
        if self.img_method == "model":
            sample_imgs1 = sample_img_prediction(self.img_model1, coord_2d1[None, :, :])[0]
        else:
            sample_imgs1 = sample_img(self.o_rgb1, coord_2d1[None, :, :])[0]
        time_profile[5], timer = refresh_timer(timer)

        # Second img
        transformation = to_homogeneous_mat_tensor(self.intrinsic2) @ self.extrinsic2 @ torch.inverse(
            self.extrinsic1)
        coord_2d2 = (transformation @ to_homogeneous_tensor(interpolated_coordinates_camera).T).T
        coord_2d2 = coord_2d2[:, :2] / coord_2d2[:, 2:3]
        valid_mask2 = torch.logical_and(coord_2d2 > 0, coord_2d2 < 1)
        valid_mask2 = torch.logical_and(valid_mask2[:, 0], valid_mask2[:, 1])
        coord_2d2 = torch.clamp(coord_2d2, 0, 1)
        if self.img_method == "model":
            sample_imgs2 = sample_img_prediction(self.img_model2, coord_2d2[None, :, :])[0]
        else:
            sample_imgs2 = sample_img(self.o_rgb2, coord_2d2[None, :, :])[0]
        time_profile[6], timer = refresh_timer(timer)
        ...
        normal_loss = (((A * solution.solution[:,0]).sum(axis=1,keepdims=True)-p[:,2:3])**2).mean() / self.seg_distance_normalizer
        ...
        if is_log and self.id_viz_patch!=-1:
            for id_edge, _ in enumerate(num_coordinates_per_edge):
                ...
                for point in polygon_2d1:
                    cv2.circle(line_img1, point, radius=3, color=(0,255,255), thickness=3)

                id_coord = num_coordinates_per_edge[:id_edge].sum()
                roi_coor_2d1_numpy = coord_2d1[
                ...
                for point in polygon_2d2:
                    cv2.circle(line_img2, point, radius=3, color=(0,255,255), thickness=3)

                id_coord = num_coordinates_per_edge[:id_edge].sum()
                roi_coor_2d2_numpy = coord_2d2[
                                     id_coord:id_coord + num_coordinates_per_edge[id_edge]].detach().cpu().numpy()
    ...
    def sample_points_2d_(self, edge_points_c, v_intrinsic, v_is_log=False, v_img=None):
        ...
        num_coordinates_per_edge = torch.ones_like(edge_points_2d[:,0,0],dtype=torch.long) * num_horizontal * num_vertical

        dx = (torch.arange(num_horizontal).to(device) / (num_horizontal - 1 + 1e-6) * length[:,None]).reshape(-1)
        dy = (torch.arange(num_vertical) - num_vertical / 2).to(device) / (num_vertical - 1 + 1e-6) * vertical_step * num_vertical
    ...
    def get_up_vector1(self, v_face_index):
        ...
        *[np.insert(np.asarray(self.edge_point_index[face_id]).reshape(-1, 4)[:, :2], 2, face_id.cpu().item(), axis=1)
    ...
    def get_up_vector2(self, v_face_index, start_points, end_points):
        ...
        t = torch.cat([self.v_up[self.face_to_up[id_face]:self.face_to_up[id_face+1]] for id_face in v_face_index], dim=0)
        t = t * 2 * math.pi
        cur_dir = end_points - start_points
        a,b,c = cur_dir[:,0],cur_dir[:,1],cur_dir[:,2]
        up_c = torch.stack((
...
class Phase3(pl.LightningModule):
    ...
    def train_dataloader(self):
        self.train_dataset = Dummy_dataset(self.model.len(), self.hydra_conf["dataset"]["only_train_target"], "training")
...
def prepare_dataset_and_model(v_colmap_dir, v_only_train_target):
    ...
    if os.path.exists(graph_cache_name):
        ...
        for i in range(1, 3):
            ...
            for id_face, id_edge_per_face in enumerate(faces):
                ...
                id_edge_per_face = [(id_edge_per_face[idx], id_edge_per_face[idx + 1]) for idx in range(len(id_edge_per_face) - 1)] + [(id_edge_per_face[-1], id_edge_per_face[0])]
    ...
    def compute_initial():
        ...
        # 1. Construct the ray
        ray_c = (np.linalg.inv(img1.intrinsic) @ np.insert(query_points, 2, 1, axis=1).T).T  # (M, 2); points in camera coordinates
    ...
    if False:
        ...
        if os.path.exists(distances_cache_file):
            ...
            ray_c = np.asarray([(graph1.nodes[id_node1]["ray_c"],graph1.nodes[id_node2]["ray_c"]) for id_node1,id_node2 in graph1.edges()])
        ...
        line_coordinates = []
        for edge in graph1.edges():
            line_coordinates.append(np.concatenate((graph1.nodes[edge[0]]["pos_world"], graph1.nodes[edge[1]]["pos_world"])))
