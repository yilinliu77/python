...
def process_file(v_id):
    try:
        if (output_root/(v_id+"_cylinder")).exists():
            return True
        # Take the ".step" file
        ...
        if len(faces) != 3 and len(faces) != 4:
            return False
        num_cylinder = 0
        num_plane = 0
        for face in faces:
            ...
            if geom_face.DynamicType().Name() == "Geom_Plane":
                num_plane += 1
            elif geom_face.DynamicType().Name() == "Geom_CylindricalSurface" or geom_face.DynamicType().Name() == "Geom_RectangularTrimmedSurface":
                num_cylinder += 1
        if num_cylinder == 1 and num_plane == 2 or num_cylinder == 2 and num_plane == 2:
            open(output_root/(v_id+"_cylinder"), "w").close()
            return True
        else:
            return False
...
if __name__ == "__main__":
    ...
    if False:
        ...
        np.savetxt("src/process_abc/abc_cylinder.txt", valid_ids, fmt="%s")
        print("Found {} cylinders".format(len(valid_ids)))