...
def process_file(v_id):
    try:
        if (output_root/(v_id+"_larger100")).exists():
            return True
        # Take the ".step" file
        ...
        if len(faces) > 100:
            open(output_root/(v_id+"_larger100"), "w").close()
            return True
        return False
...
if __name__ == "__main__":
    ...
    if False:
        ...
        np.savetxt("src/process_abc/abc_larger100.txt", valid_ids, fmt="%s")
        print("Found {} larger100".format(len(valid_ids)))