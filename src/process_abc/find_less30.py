...
def process_file(v_id):
    try:
        if (output_root/(v_id+"_less30")).exists():
            return True
        # Take the ".step" file
        ...
        if len(faces) < 30:
            open(output_root/(v_id+"_less30"), "w").close()
            return True
        return False
...
if __name__ == "__main__":
    ...
    if False:
        ...
        np.savetxt("src/process_abc/abc_less30.txt", valid_ids, fmt="%s")
        print("Found {} less30".format(len(valid_ids)))