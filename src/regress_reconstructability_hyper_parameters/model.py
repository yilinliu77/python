...
class Correlation_l2_error_net2(Correlation_net):
    def __init__(self, hparams):
        ...
        self.view_feature_fusioner2 = TFEncorder(256, 2, 512, 0.2, batch_first=True,
                                                 add_bias_kv=self.hydra_conf["model"]["add_bias_kv"])
    ...
    def forward(self, v_data: Dict[str, torch.Tensor]):
        ...
        valid_mask = view_attribute[:, :, 0].type(torch.bool)  # Filter out the unusable view

        fused_features_per_view, fused_weights = self.view_feature_fusioner1(
            predicted_recon_error_per_view,
            src_key_padding_mask = torch.logical_not(valid_mask)
        )
        fused_features_per_view = fused_features_per_view * valid_mask[:,:,None]

        fused_features_with_token_per_view = torch.cat([
            torch.tile(self.magic_class_token, [fused_features_per_view.shape[0], 1, 1]),
            fused_features_per_view
        ], dim=1)
        ...
        # weights = weights[:, 0]
        fused_view_features = fused_view_features * valid_mask[:,:,None]
