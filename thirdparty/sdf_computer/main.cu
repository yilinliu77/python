    ...
    const int v_num_uniform)
{
    ...
    const uint32_t num_point = v_camera_pos.size();
    ...
    tcnn::GPUMemory<int> visibility;
    visibility.enlarge(num_point);

    triangle_bvh->check_visibility(num_point,
        camera_pos_gpu.data(),
        point_pos_gpu.data(),
        triangles_gpu.data(),
        visibility.data(),
    ...
    std::vector<int> results(num_point);
...
public:
    ...
    void setup_mesh(const std::vector<Eigen::Vector3f>& v_vertices)
    {
...
int main(int argc, char** argv)
{
    ...
    cameras.emplace_back(1,1,1);
    points.emplace_back(0,0,0);

    load_mesh2(
        cameras,
        points,
        "D:/Projects/NeuralRecon/Test_data/OBL_L7/Test_imgs2_colmap_neural/sparse_align/detailed_l7_with_ground.obj",
        40000, 30000, 10000
    );
}
