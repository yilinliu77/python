    ...
    short* __restrict__ visibility)
{
        ...
        const uint32_t i_camera = i / num_point;
        const uint32_t i_point = i % num_point;

        Vector3f dir = v_point_poses[i_point] - v_camera_poses[i_camera];
        float distance = dir.norm();
        dir = dir / distance;
        auto result = TriangleBvh4::ray_intersect(
            v_camera_poses[i_camera],
            dir,
            bvh_nodes,
            v_triangles);
